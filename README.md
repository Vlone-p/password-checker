...
We use a concept called k-Anonymity using the SHA-1 hashing algorithm:
    ...
    Your browser hashes your password locally using the Web Crypto API.
    Only the first 5 characters of that hash are sent to the HaveIBeenPwned (HIBP) API.
    The API returns a list of hundreds of leaked password hashes that start with those 5 characters.
    Your browser checks if your full hash is in that list locally.Result: You know if your password is leaked, but HIBP never saw your password.
...
🛠️ Tech Stack
    ...
    Vanilla JavaScript - DOM manipulation, async/await for API calls
    Web Crypto API - For secure, local SHA-1 hashing

🚀 How to Run Locally

Because this project uses the Web Crypto API, it must be served over a local web server (opening the index.html file directly via file:// might block the API).
